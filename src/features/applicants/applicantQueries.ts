...
import { applicants, users, resumes, jobApplications, jobs, employers } from "@/drizzle/schema";

export async function getApplicantProfileData(userId: number) {
  const [combinedData] = await db.select({
      user: users,
      applicant: applicants,
      resume: resumes,
    }).from(users).leftJoin(
        applicants, eq(users.id, applicants.id)
    ).leftJoin(
        resumes, eq(applicants.id, resumes.applicantId)
    ).where(eq(users.id, userId)).limit(1); // Object Based Projection

  // If no user is found, combinedData will be undefined
  if (!combinedData || !combinedData.user){
    return null;
  }
...
export async function getAppliedJobsForApplicant(userId: number) {
  const applications = await db.select({
      ...
      job: jobs,
      employer: employers,
    }).from(jobApplications).innerJoin(
      jobs, eq(jobApplications.jobId, jobs.id)
    ).leftJoin(
      employers, eq(jobs.employerId, employers.id)
    ).where(
