...
import { getAppliedJobsForApplicant } from "../applicantQueries";
import { getCurrentUser } from "@/features/auth/authQueries";
import { redirect } from "next/navigation";
import { format } from "date-fns/format";

export async function RecentlyAppliedJobs() {
        ...
        <Table>
          ...
          <TableBody>
            {recentApplications.map((appliedJob) => {
              const { application, job, employer } = appliedJob;

              return (<TableRow key={application.id} className="hover:bg-gray-50">
                {/* Job Info Column */}
                <TableCell className="pl-6 py-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-xs font-bold text-gray-500 overflow-hidden border">
                      {employer?.bannerImageUrl ? (<Image src={employer.bannerImageUrl} alt={employer.name || "Company"} fill className="object-cover" />
                      ) : (
                ...
                <TableCell className="text-right pr-6">
                  <Button
                    variant="secondary"
                    size="sm"
                    className="bg-gray-100 hover:bg-gray-200 text-blue-600 font-medium" asChild>
