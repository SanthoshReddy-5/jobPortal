...
import { JobCardType } from '../jobQueries';
...
const JobCard = ({ job }: JobCardProps) => {
    ...
    return (
        <Link href={`/dashboard/jobs/${job.id}`} className='group flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-all hover:border-blue-500/50 hover:shadow-md'>
            <div className='flex items-start justify-between gap-4'>
                <div className='flex gap-3'>
                    <div className='relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg border border-gray-100 bg-gray-50'>
                        {job.companyLogo ? (<Image src={job.companyLogo} alt={job.companyName || "Company Logo"} fill className="object-cover" />) : (
                    ...
                    <div>
                        <h3 className='max-w-[180px] truncate font-semibold text-gray-900 group-hover:text-blue-600'>{job.title}</h3>
            ...
            <div className='mt-auto flex items-center justify-between border-t border-gray-100 text-xs text-gray-500'>
                ...
                {/* <span className='font-medium text-blue-600 group-hover:underline'>View Details &rarr;</span> */}
            </div>
        </Link>
    )
}
