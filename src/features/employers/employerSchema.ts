...
export const employerProfileSchema = z.object({
    name: z.string().trim().min(2, "Company name must be at least 2 characters long").max(255, "Company name must not exceed 255 characters"),
    description: z.string().trim().min(10, "Description must be at least 10 characters long").max(2000, "Description must not exceed 2000 characters"),
    organizationType: z.enum(organizationTypeOptions, {error: "Please select a valid organization type",}),
    teamSize: z.enum(teamSizeOptions, { error: "Please select a valid team size" }),
    yearOfEstablishment: z.string().trim().regex(/^\d{4}$/, "Please enter a valid 4-digit year").refine(
      (year) => {
