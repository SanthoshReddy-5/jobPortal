...
async function getFeaturedJobs() {
  return await db.select({ 
    job: jobs, employer: employers 
  }).from(jobs).leftJoin(employers, eq(
      ...
      <footer className="bg-gray-900 text-gray-300 py-12">
        <div className="container mx-auto max-w-7xl px-4 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2 font-bold text-xl text-white">
            <Briefcase className="w-6 h-6" /> JobAtlas
          </div>
          <p className="text-sm">© {new Date().getFullYear()} Santhosh Reddy.</p>
        </div>
      </footer>
    </div>
  );
